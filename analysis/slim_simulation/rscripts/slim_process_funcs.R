...
SLiM_validate <- function(PLOD, 
 ...
 # k$pop2 <- NULL
  ...
  if (!is.null(pcrelate)) {
    ...
    k2 <- rbind(merge(k2,  k3[,c("ID1","ID2","status")], 
                     by.x=c("ID1","ID2"), 
                     by.y=c("ID1","ID2")),
               merge(k2,  k3[,c("ID1","ID2","status")], 
                     by.x=c("ID1","ID2"), 
                     by.y=c("ID2","ID1")))
    names(k2)[names(k2) == 'status'] <- 'estkinship.PC'
  }
...
findkinships <- function(pedigree, k, sampled = NULL) {
  ...
  print("Profiling parent-offspring pairs")
  ...
  #table(k2$kinship)
  # 
  # kks.sub <- kks[(kks$kin > 0.09  & kks$kin < 0.15) &
  #               (kks$k0  > 0.35  & kks$k0 < 0.65)
  #                & (kks$kinship != "HSP") , ]
  
  #kks <- kks.sub
  #max(abs(kks$age1 - kks$age2))
  
  for (i in seq(1, dim(kks)[1]))
  {
    if (i %% 200 == 0) {print(i)}
    p1 <- pedigree[id == kks$longID1[i], ]
    p2 <- pedigree[id == kks$longID2[i], ]
    ...
    # Test - FTPs
    if ( (sum(p1.gs %in% p2.ps) == 2 & sum(p1.ps %in% p2$id) != 1) |
         (sum(p2.gs %in% p1.ps) == 2 & sum(p2.ps %in% p1$id) != 1) )
    {
      print(i)
      kks$kinship[i] <- "FTP"
