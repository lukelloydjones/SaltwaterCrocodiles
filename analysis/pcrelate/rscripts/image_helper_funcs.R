# ==============================================================================
# Image generation for telemetry data helper functions
# Author: Luke Lloyd-Jones
# Date started: 30/03/2022
# Date updated: 30/03/2022
# ==============================================================================

# ----------------------------------
# Function to cycle over points 
# files and put into one spatial 
# points data frame
# ---------------------------------- 

makeBigPoints <- function(ctrl.pth, out.pth, out.nm)
{
  require(sp) # Need spatial points package
  
  # E.g., paths s=
  #  ctrl.pth <- "data/waterbirdscontrolfile.csv"
  #  out.pth  <- "out/"
  #  out.nm   <- "all_birds_all_points"
  
  ctrl.f  <- read.csv(ctrl.pth, header = T)

  for(i in seq(1, dim(ctrl.f)[1]))
  {
    bird     <- ctrl.f$BirdID[i]
    brd.in   <- paste0(out.pth, bird, "_points.csv")
    if (i == 1)
    {
      pts.all <- read.csv(brd.in, header = T)
      pts.all$SpCode <- ctrl.f$SpCode[i]
    } else {
      pts.i <- read.csv(brd.in, header = T)
      pts.i$SpCode <- ctrl.f$SpCode[i]
      pts.all <- rbind(pts.all, pts.i) 
    }
  }

  # Make spatial point data frame

  all.brd.sp <- SpatialPointsDataFrame(pts.all[, c("Longitude", "Latitude")], 
                                       pts.all)

  # Easier as data frame for some things

  all.brd.sp.df <- data.frame(all.brd.sp)
  
  # Save
  print(paste0("Saving big point file to file to "), paste0(out.pth, out.nm))

  save(all.brd.sp.df, file = paste0(out.pth, out.nm, ".Rda"))
  save(all.brd.sp,    file = paste0(out.pth, out.nm, "_sp.Rda"))
}

# ----------------------------
# Function to grab pngs
# ----------------------------
...
scaleBar <- function(lon, lat, distanceLon, distanceLat, distanceLegend, dist.unit = "km", 
                     rec.fill = "white", rec.colour = "black", rec2.fill = "black", rec2.colour = "black", 
                     legend.colour = "black", legend.size = 3, orientation = TRUE, arrow.length = 500, arrow.col = "white",
                     arrow.distance = 300, arrow.North.size = 6)
{
  ...
  if(orientation){# Add an arrow pointing North
    ...
    arrow <- list(geom_segment(data = coordsArrow$res, 
                               ...
                               colour = arrow.col), 
                  annotate("text", label = "N", x =
                             coordsArrow$coordsN[1,"x"], 
                           y = coordsArrow$coordsN[1,"y"], 
                           size = arrow.North.size, colour = arrow.col))
    res <- c(res, arrow)
  }
  return(res) }
